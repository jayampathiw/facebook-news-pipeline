...
src/renderers/reel.js  (Node.js orchestrator)
  ...
  │     OR image_prompt → fal.ai (primary) → Cloudflare → Google
  │     → /tmp/reels/<id>/bg.png
  │
  ├── Step 4: Music ──────────────────────────────────────────────
...
2. **`article.image_prompt`** → generate via provider chain:
   - `fal.ai` (primary) — Recraft V3, 1080×1920
   - `cloudflare` (fallback) — Flux Schnell, `width:1080, height:1920`
   - `google` (secondary fallback) — Gemini image generation
   - `pollinations` (last resort) — Pollinations.ai FLUX model
...
| Subtitle `FontSize` | `13` | ASS virtual-pixel size |
| Subtitle `MarginV` | `80` | Bottom margin in ASS virtual pixels |
| Subtitle `MarginL` / `MarginR` | `40` each | Horizontal safe zone |
...
|---|---|---|---|
| `FAL_KEY` | primary | — | fal.ai API key (Recraft V3, primary image provider) |
| `CF_ACCOUNT_ID` | fallback | — | Cloudflare account ID |
| `CF_API_TOKEN` | fallback | — | Cloudflare API token |
| `GOOGLE_AI_KEY` | fallback | — | Google AI Studio API key |
...
Edit `FontSize` in the `subStyle` string in `compositeReel()`. With the default libass PlayRes of 384×288, the rendered font at 1920px height is approximately `FontSize × 6.7`:
...
Increase `MarginL` and `MarginR` in `subStyle`. Current value is `40` each (~112px from left edge at 1080px width).
...
3. Add Facebook credentials to `.env` and GitHub Secrets: `FB_PAGE_ID_XX`, `FB_ACCESS_TOKEN_XX`.
...
- Read the full ffmpeg stderr — it always names the specific filter or argument that failed
- Common causes: subtitle path with special characters, logo file missing, music file shorter than reel

### Subtitles not appearing in output
- Check `libass` is compiled into your FFmpeg: `ffmpeg -filters | grep subtitles`
- Verify the SRT file was created: check Whisper's output dir
...
| File | Title | License |
|---|---|---|
| `breaking.mp3` | Investigations | CC BY 4.0 |
| `standard.mp3` | Chill Wave | CC BY 3.0 |
| `positive.mp3` | Upbeat Forever | CC BY |
