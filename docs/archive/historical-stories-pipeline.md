...
### Engagement question per mode

| Mode | Question |
|---|---|
| ORGOGLIO | *Di cosa sei più orgoglioso/a dell'Italia?* |
| PATRIMONIO | *Cosa trasmetti di italiano ai tuoi figli?* |
| FIERTÉ | *Qu'est-ce qui vous rend le plus fier(ère) d'être français(e)?* |
| PATRIMOINE | *Quel est votre patrimoine français préféré?* |
...
| `seed_comment_template_id` | Template ID used |
...
| `pillar` | Content pillar hint from Claude |
...
### Row identification
Historical rows are visually distinct from news rows:
...
### Source filter
A **Source** dropdown in the filter toolbar lets you isolate them:
- `All Sources` — default, shows everything
- `📰 News` — hides historical rows
- `🏛️ Historical` — shows only historical rows
...
List all available topic IDs for a country by passing an invalid ID — the error message prints the full list.
...
### Topic content rules
The backlog is intentionally constrained to keep content safe and broadly appealing:
...
Historical articles use `historical://<topic-id>` as their `original_url`. This is intentional — it serves as the dedup key and prevents the pipeline from treating duplicate fetches as separate articles. Do not replace it with a Wikipedia link or any real URL.
...
2. Add an object to `HISTORICAL_STORIES` following this shape:
...
3. No other changes needed — the selection logic and queue script pick it up automatically.
...
| `src/utils/historicalTopics.js` | `pickNextTopic()` selection logic + `topicToOriginalUrl()` |
